...
test.describe('[M05] Permissions - RBAC Enforcement', () => {
  ...
  test('TC_PERM_009 - Employee bị chặn /admin/permissions', async ({ employeePage: page }) => {
    ...
    const redirected = !page.url().includes('/admin/permissions');
  ...
  test('TC_PERM_010 - Employee bị chặn /admin/payroll/config', async ({ employeePage: page }) => {
    ...
    await page.waitForTimeout(2000);
    const denied = await page.getByText(/Access Denied|Truy cập bị từ chối/i).isVisible().catch(() => false);
    const redirected = !page.url().includes('/admin/payroll/config');
    expect(denied || redirected).toBeTruthy();
  });
  ...
  test('TC_PERM_012 - Employee bị chặn /admin/resignations', async ({ employeePage: page }) => {
    ...
    await page.waitForTimeout(2000);
    const denied = await page.getByText(/Access Denied|Truy cập bị từ chối/i).isVisible().catch(() => false);
    const redirected = !page.url().includes('/admin/resignations');
    expect(denied || redirected).toBeTruthy();
  });
