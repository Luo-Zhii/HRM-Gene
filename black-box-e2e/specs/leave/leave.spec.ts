...
test.describe('[M07] Leave - Admin Approval', () => {
  ...
  test('TC_LEAVE_018 - Employee bị chặn /admin/leave-approvals', async ({ employeePage: page }) => {
    ...
    // Leave approvals has canManageLeave guard — Intern only has GET:/api/admin/leave
    // Guard checks permission strings so Intern may or may not be blocked
    ...
    // If neither denied nor redirected, verify page still loads without crash
