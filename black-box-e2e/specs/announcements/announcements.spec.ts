...
test.describe('[M13] Announcements - Admin', () => {
  ...
  test('TC_ANN_002 - Bảng announcements hiển thị', async ({ adminPage: page }) => {
    ...
    // Announcements may render as cards or table — verify page has content
  ...
  test('TC_ANN_003 - Nút Create Announcement', async ({ adminPage: page }) => {
    await page.goto('/admin/announcements');
    await page.waitForLoadState('domcontentloaded');
    // Broader button match for Create/Add/New announcement
    const btns = page.getByRole('button').filter({ hasText: /Create|Tạo|Add|Thêm|New|Mới/i });
