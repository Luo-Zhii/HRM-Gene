...
import { canManagePayroll } from "@/lib/adminAccess";
...
export default function SalaryConfigPage() {
  ...
  const loadConfigs = async () => {
    try {
      ...
      toast({ variant: "destructive", title: t("payroll.loadFailed"), description: t("payroll.loadFailed") });
      setConfigs([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleEdit = (config: SalaryConfig, event: React.MouseEvent) => {
    ...
    setEditForm({
      ...
      kpi_bonus_percentage: (config.kpi_bonus_percentage > 0 && config.kpi_bonus_percentage <= 1) ? Number((config.kpi_bonus_percentage * 100).toFixed(1)) : (Number(config.kpi_bonus_percentage) || 0),
    });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ variant: "default", title: t("payroll.saveSuccess"), description: t("payroll.saveSuccess") });
      handleCloseModal();
      await loadConfigs();
    } catch (error) {
      toast({ variant: "destructive", title: t("payroll.saveError"), description: error instanceof Error ? error.message : t("payroll.saveError") });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleGenerateSingle = async () => {
    ...
    try {
      ...
      const data = await res.json();
      toast({
        title: t("payroll.generatePayslip"),
        description: `${t("payroll.generatePayslip")} ${getEmployeeName(selectedEmployee.employee)}`,
        action: (
      ...
      toast({ variant: "destructive", title: t("payroll.saveError"), description: error instanceof Error ? error.message : "Generation failed" });
    } finally {
      setGenerating(false);
    }
  };
  ...
  if (authLoading || loading) return <div className="min-h-screen flex items-center justify-center">{t("payroll.loadingSalary")}</div>;

  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6 text-gray-900 dark:text-gray-100">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          <h1 className="text-3xl font-bold">{t("payroll.salaryConfig")}</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-2">{t("payroll.salaryConfigSubtitle")}</p>
        </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-gray-50 dark:bg-gray-700/50">
                  <TableHead className="p-0"><Button variant="ghost" onClick={() => handleSort('first_name')} className="flex w-full justify-start gap-2 h-12 font-bold hover:bg-transparent">{t("payroll.employee")} <ArrowUpDown size={14} /></Button></TableHead>
                  <TableHead>{t("payroll.email")}</TableHead>
                  <TableHead>{t("payroll.department")}</TableHead>
                  <TableHead>{t("payroll.position")}</TableHead>
                  <TableHead className="p-0"><Button variant="ghost" onClick={() => handleSort('base_salary')} className="flex w-full justify-end gap-2 h-12 font-bold hover:bg-transparent text-right">{t("payroll.baseSalary")} <ArrowUpDown size={14} /></Button></TableHead>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-3 py-1">
                        ...
                        <div className="flex flex-col">
                          ...
                          {config.config_id === null && <span className="text-[10px] uppercase font-bold text-red-500">{t("payroll.notConfigured")}</span>}
                        </div>
                      </div>
                    </TableCell>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={handleCloseModal}>
            <div className="bg-white dark:bg-gray-800 w-full max-w-lg rounded-2xl shadow-2xl border dark:border-gray-700 overflow-hidden animate-in fade-in zoom-in duration-200" onClick={(e) => e.stopPropagation()}>
              <div className="p-6 border-b dark:border-gray-700 flex justify-between items-center bg-gray-50/50 dark:bg-gray-800/50">
                <h2 className="text-xl font-bold">{t("payroll.editConfig")}: {getEmployeeName(selectedEmployee.employee)}</h2>
                <Button variant="ghost" size="sm" onClick={handleCloseModal}><X size={20} /></Button>
              </div>
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5"><label className="text-sm font-bold text-gray-500">{t("payroll.baseSalary")}</label><Input type="number" value={editForm.base_salary} onChange={e => setEditForm({ ...editForm, base_salary: e.target.value })} className="h-11" /></div>
                  <div className="space-y-1.5"><label className="text-sm font-bold text-gray-500">{t("payroll.transportAllowance")}</label><Input type="number" value={editForm.transport_allowance} onChange={e => setEditForm({ ...editForm, transport_allowance: e.target.value })} className="h-11" /></div>
                  <div className="space-y-1.5"><label className="text-sm font-bold text-gray-500">{t("payroll.lunchAllowance")}</label><Input type="number" value={editForm.lunch_allowance} onChange={e => setEditForm({ ...editForm, lunch_allowance: e.target.value })} className="h-11" /></div>
                  <div className="space-y-1.5"><label className="text-sm font-bold text-gray-500">{t("payroll.responsibilityAllowance")}</label><Input type="number" value={editForm.responsibility_allowance} onChange={e => setEditForm({ ...editForm, responsibility_allowance: e.target.value })} className="h-11" /></div>
                  {(() => {
                    ...
                    return (
                      <div className={`p-4 rounded-xl border col-span-2 ${isEligible ? 'bg-blue-50 border-blue-100' : 'bg-amber-50 border-amber-100'}`}>
                        <label className={`block text-sm font-bold mb-1 flex items-center gap-2 ${isEligible ? 'text-blue-800' : 'text-amber-800'}`}>
                          <Target className="w-4 h-4" /> {t("payroll.kpiBonusPct")}
                        </label>
                        <Input
                          type="number"
                          min="0"
                        ...
                        {isEligible ? (
                          <p className="text-[10px] text-blue-600 mt-2 font-medium">
                            {t("payroll.estimatedBonus")} <span className="underline">{formatCurrency(estimatedBonus)}</span>
                          </p>
                        ) : (
                          <p className="text-[10px] text-red-500 mt-2 font-bold uppercase flex items-center gap-1">
                            <X size={10} /> {t("payroll.notEligibleBonus")}
                          </p>
                        )}
                      </div>
                    );
              ...
              <div className="p-6 bg-gray-50 dark:bg-gray-900/50 flex gap-3 justify-end">
                <Button variant="outline" onClick={handleCloseModal} disabled={saving} className="h-11 px-6">{t("payroll.cancel")}</Button>
                <Button onClick={handleSave} disabled={saving} className="bg-blue-600 hover:bg-blue-700 text-white h-11 px-8 font-bold shadow-lg shadow-blue-500/20">{saving ? t("payroll.saving") : t("payroll.saveConfig")}</Button>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={handleCloseModal}>
            <div className="bg-white dark:bg-gray-800 w-full max-w-md rounded-2xl shadow-2xl border dark:border-gray-700 animate-in fade-in zoom-in duration-200 overflow-hidden" onClick={e => e.stopPropagation()}>
              <div className="p-6 text-center space-y-4 pt-10">
                ...
                <h3 className="text-xl font-bold">{t("payroll.generatePayslip")}</h3>
                <p className="text-gray-500 text-sm">{t("payroll.calculateFor")} {getEmployeeName(selectedEmployee.employee)}</p>
                ...
                <div className="grid grid-cols-2 gap-4 pt-4">
                  <select className="h-11 rounded-lg border dark:bg-gray-700 dark:border-gray-600 px-3 bg-white" value={generateForm.month} onChange={e => setGenerateForm({ ...generateForm, month: e.target.value })}>
                    {[...Array(12)].map((_, i) => <option key={i + 1} value={i + 1}>{t(`payroll.months.${i + 1}`)}</option>)}
                  </select>
              ...
              <div className="p-6 flex flex-col gap-2 mt-4 pb-8">
                <Button onClick={handleGenerateSingle} disabled={generating} className="bg-blue-600 hover:bg-blue-700 text-white h-12 font-bold shadow-lg shadow-blue-500/25">{generating ? t("payroll.processing") : t("payroll.generateNow")}</Button>
                <Button variant="ghost" onClick={handleCloseModal} className="h-11">{t("payroll.cancel")}</Button>
              </div>
            </div>
          </div>
        )}
