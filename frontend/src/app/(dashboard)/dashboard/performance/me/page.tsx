  ...
  return (
    <div className="space-y-8 pb-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight flex items-center gap-3 italic uppercase">
            <Zap className="w-8 h-8 text-blue-600 fill-blue-600" /> {t("performance.myPerformance")}
          </h1>
          <p className="text-sm text-gray-500 font-medium mt-1">{t("performance.myPerformanceDescription")}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="rounded-3xl border-none shadow-xl shadow-blue-50 bg-blue-600 text-white overflow-hidden relative group">
            ...
            <CardHeader className="pb-2 relative">
              <CardTitle className="text-[10px] font-black uppercase tracking-[0.2em] opacity-80">{t("performance.weightedKpiScore")}</CardTitle>
            </CardHeader>
            <CardContent className="relative">
              ...
              <p className="text-[10px] mt-4 font-bold text-blue-100 flex items-center gap-1">
                <CheckCircle2 className="w-3 h-3" /> {t("performance.weightedAverageGoals")}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-3xl border-none shadow-xl shadow-emerald-50 bg-emerald-500 text-white overflow-hidden relative group">
            <div className="absolute -right-8 -top-8 w-32 h-32 bg-white/10 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-700" />
            <CardHeader className="pb-2 relative">
              <CardTitle className="text-[10px] font-black uppercase tracking-[0.2em] opacity-80">{t("performance.bonusMultiplier")}</CardTitle>
            </CardHeader>
            <CardContent className="relative">
              <div className="flex items-end gap-2">
              ...
              <p className="text-[10px] mt-4 font-bold text-emerald-50 flex items-center gap-1">
                <ArrowUpRight className="w-3 h-3" /> {t("performance.multiplierTargetBonus")}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-3xl border-gray-100 shadow-xl shadow-gray-100 bg-white overflow-hidden flex flex-col justify-center px-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[10px] font-black uppercase text-gray-400 tracking-widest leading-none mb-1">{t("performance.status")}</p>
                <p className="text-lg font-black text-gray-900 uppercase italic">{t("performance.reviewPending")}</p>
              </div>
            </div>
          </Card>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {loading ? (
          <div className="col-span-full py-20 text-center animate-pulse text-gray-400 font-bold uppercase tracking-widest">{t("performance.syncingPerformance")}</div>
        ) : assignments.length === 0 ? (
          <div className="col-span-full bg-white rounded-[2rem] border-2 border-dashed border-gray-100 py-24 flex flex-col items-center justify-center text-gray-400">
            ...
            <p className="font-bold text-gray-900 uppercase tracking-widest text-sm mb-1">{t("performance.noGoalsFound")}</p>
            <p className="text-xs">{t("performance.noKpisAssignedYet")}</p>
          </div>
        ) : (
          assignments.map(a => {
            const score = a.manager_score !== null && a.manager_score !== undefined ? a.manager_score : a.actual_value;
            const achievement = calculateAchievement(score, a.target_value);
              ...
              <div key={a.id} className="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-gray-100 border border-gray-50 group hover:shadow-2xl hover:shadow-blue-50 transition-all duration-500 hover:-translate-y-1">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="bg-blue-50 text-blue-600 text-[10px] font-black px-2 py-0.5 rounded-full uppercase tracking-tighter">{t("performance.metricGoal")}</span>
                      <span className="text-[10px] font-black text-gray-300 uppercase italic">{t("performance.weight")}: {a.weight}%</span>
                    </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between text-[10px] font-black uppercase text-gray-400 tracking-widest">
                    <span>{t("performance.performanceProgress")}</span>
                ...
                <div className="mt-8 flex items-center justify-between pt-6 border-t border-gray-50">
                  ...
                  {isSelfUpdate ? (
                    <div className="flex items-center gap-2">
                      <Input
                        type="number"
                        placeholder={t("performance.newValue")}
                        value={updateValue}
                        onChange={(e) => setUpdateValue(e.target.value)}
                        className="w-24 h-9 font-bold bg-white"
                      />
                      <Button size="sm" onClick={() => handleUpdateActual(a.id)} className="bg-blue-600 h-9 font-black uppercase tracking-widest text-[10px] px-4">{t("common.save")}</Button>
                    ...
                    >
                      {t("performance.updateProgress")} <ArrowUpRight className="w-3.5 h-3.5 ml-1 transition-transform group-hover/btn:-translate-y-0.5 group-hover/btn:translate-x-0.5" />
                    </Button>
                  )}
                </div>
              </div>
