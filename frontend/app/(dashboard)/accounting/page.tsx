...
export default function AccountingPage() {
  ...
  const runPayroll = async () => {
    ...
    try {
      const res = await fetch(
        `/api/payroll/run?month=${parseInt(month, 10)}&year=${parseInt(year, 10)}`,
  ...
  const salaryComponentsData = data
    ? [
        { name: t("payslip.baseSalary"), value: data.total_base_salary },
        { name: t("payslip.bonusCommission"), value: data.total_bonus },
        { name: t("payslip.totalDeductions"), value: data.total_deductions },
      ]
    : [];
    ...
    <div className="p-6">
      ...
      <div className="mt-6 flex items-center justify-between gap-6">
        ...
        {hasManagePayroll && (
          <div className="p-4 bg-white rounded shadow flex flex-col gap-2 min-w-fit">
            <label className="text-sm font-semibold">{t("accounting.period")}</label>
            <div className="flex gap-2 items-end">
              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-600">{t("accounting.month")}</label>
                <input
                  type="number"
                  min="1"
                  max="12"
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-600">{t("accounting.year")}</label>
                <input
                  type="number"
              ...
              <button
                ...
                disabled={running}
              >
                {running ? t("accounting.running") : t("accounting.runPayroll")}
              </button>
            </div>
          </div>
        )}
      ...
      <div className="mt-8 grid grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded shadow p-6">
          <h2 className="text-lg font-semibold mb-4">{t("accounting.payrollByDept")}</h2>
          {data?.payroll_by_department && data.payroll_by_department.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={data.payroll_by_department}>
                ...
                <Tooltip />
                <Legend />
                <Bar dataKey="total" fill="#3b82f6" name={t("accounting.totalPayrollLabel")} />
                <Bar dataKey="count" fill="#10b981" name={t("accounting.employeeCount")} />
              </BarChart>
            </ResponsiveContainer>
          ) : (
            <div className="h-48 flex items-center justify-center text-gray-500">
              {t("accounting.noDeptData")}
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded shadow p-6">
          <h2 className="text-lg font-semibold mb-4">{t("accounting.salaryBreakdown")}</h2>
